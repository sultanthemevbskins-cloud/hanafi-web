...
const PHONE_PATH =
  'M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.42 2 2 0 0 1 3.6 1.25h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.91a16 16 0 0 0 6.06 6.06l.97-.97a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 21.73 16.92z'
...
export default function ContactForm({ onOpenContact }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (json.success) { setStatus('success'); formRef.current?.reset() }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-10 lg:gap-16 items-start">
          ...
          <div className="flex flex-col gap-8">
            ...
            <div>
              ...
              <p className="font-poppins text-[15px] text-[#374151] leading-relaxed">
          ...
          {/* ══ RIGHT COLUMN — Call Us card + FAQs ══ */}
          <div className="flex flex-col gap-6">
            ...
            <div className="rounded-[16px] border border-[#e5e7eb] bg-white overflow-hidden shadow-sm">
              ...
              <div className="flex flex-col divide-y divide-[#f3f4f6]">
                {faqs.slice(0, 3).map((q, i) => <FaqRow key={i} question={q} />)}
              </div>

              {/* Collapsible remaining 7 */}
              <div
                className="overflow-hidden transition-all duration-300 ease-in-out"
                style={{ maxHeight: faqOpen ? `${(faqs.length - 3) * 52}px` : '0px' }}
              >
                <div className="flex flex-col divide-y divide-[#f3f4f6] border-t border-[#f3f4f6]">
                  {faqs.slice(3).map((q, i) => <FaqRow key={i + 3} question={q} />)}
                </div>
              </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-5 py-3 border-t border-[#f3f4f6] hover:bg-[#f9fafb] transition-colors"
              >
